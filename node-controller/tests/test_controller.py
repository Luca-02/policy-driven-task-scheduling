...
class TestSanitizeNode(unittest.TestCase):
    ...
    def test_empty_lambda_is_untouched(self):
        self.ctrl._v1.read_node.return_value = self._node(annotations={})
        self.ctrl.sanitize_node("n1", self.logger)
        self.ctrl._v1.patch_node.assert_not_called()

    def test_empty_list_lambda_is_untouched(self):
        self.ctrl._v1.read_node.return_value = self._node(annotations={self.key: "[]"})
        self.ctrl.sanitize_node("n1", self.logger)
        self.ctrl._v1.patch_node.assert_not_called()
    ...
    def test_active_pod_leaves_taint_applied_and_lambda_untouched(self):
        ...
        self.ctrl._v1.list_pod_for_all_namespaces.return_value = MagicMock(
        ...
        patch_calls = self.ctrl._v1.patch_node.call_args_list
    ...
    def _wire_fake(self, annotations=None, taints=None):
        ...
        self.ctrl._v1.read_node.side_effect = fake.read_node
        self.ctrl._v1.patch_node.side_effect = fake.patch_node
        return fake
    ...
    def test_ignores_succeeded_and_failed_pods(self):
        ...
        self.assertEqual(len(self.ctrl._v1.patch_node.call_args_list), 3)
        self.assertNotIn(self.key, fake.annotations)

    def test_full_sanitization_clears_lambda_and_removes_taint(self):
        fake = self._wire_fake(annotations={self.key: '["Ford"]'})
        self.ctrl._v1.list_pod_for_all_namespaces.return_value = MagicMock(items=[])
...
class TestHasActivePods(unittest.TestCase):
    ...
    def test_no_pods(self):
        self.ctrl._v1.list_pod_for_all_namespaces.return_value = MagicMock(items=[])
        self.assertFalse(self.ctrl._has_active_pods("n1", self.logger))

    def test_running_pod_from_our_scheduler(self):
        self.ctrl._v1.list_pod_for_all_namespaces.return_value = MagicMock(
            items=[make_pod("t1", self.config.scheduler_name, "Running")]
        )
        self.assertTrue(self.ctrl._has_active_pods("n1", self.logger))
    ...
    def test_uses_correct_field_selector(self):
        self.ctrl._v1.list_pod_for_all_namespaces.return_value = MagicMock(items=[])
        self.ctrl._has_active_pods("kind-worker", self.logger)
        self.ctrl._v1.list_pod_for_all_namespaces.assert_called_once_with(
