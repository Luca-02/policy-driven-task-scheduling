...
class TestSanitizeNodeDelegatesToSanitizer(unittest.TestCase):
    def setUp(self):
        self.config = make_config()
        self.ctrl = make_ctrl(self.config)
    ...
    def test_delegates_to_sanitizer(self):
        self.ctrl._sanitizer = MagicMock()
        self.ctrl.sanitize_node("n1", self.logger)
        self.ctrl._sanitizer.sanitize_node.assert_called_once_with("n1", self.logger)
