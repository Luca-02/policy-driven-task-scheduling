...
class SanitizationStrategy(str, Enum):
    ...
    NONE:               never sanitize.
    PERIODIC:           every ``period`` steps, all nodes with a non-empty
                        footprint.
    THRESHOLD:          nodes whose footprint reaches ``threshold`` contexts,
                        checked at every step.
    PERIODIC_THRESHOLD: both triggers at once, in union. A node is sanitized
                        as soon as its footprint reaches the threshold, and
                        in any case at the periodic sweep. Combines a
                        guaranteed upper bound on how long a trace survives
                        with an early reaction to the nodes that accumulate
                        fastest.
    """
...
class DatasetConfig:
    ...
    ``n_datasets`` is sized so that every issuer can form the longest request
    the experiments use. With ``auth_size`` = 1 an issuer may only request the
    datasets of its own context plus the public ones, so the population has to
    be large enough that ``n_datasets * (1 - public_fraction) / n_contexts +
    n_datasets * public_fraction`` comfortably exceeds the largest
    ``req_max``. Sizing it too tightly does not merely risk an error: it
    silently restricts the batch to the few issuers that happen to qualify,
    concentrating the contexts and biasing every result.
    """
