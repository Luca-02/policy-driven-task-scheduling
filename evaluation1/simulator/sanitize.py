...
def select_for_sanitization(state: ClusterState, cfg: SanitizationConfig) -> list[Node]:
    ...
    THRESHOLD: only nodes whose footprint has reached ``threshold``
               ...
               actually large, but leaves a lightly contaminated node dirty
               indefinitely.
    PERIODIC_THRESHOLD: the union of the two. The threshold reacts early to
               the nodes that accumulate fastest, while the period puts a
               ceiling on how long any trace can survive.
    """
    ...
    if cfg.strategy is SanitizationStrategy.PERIODIC_THRESHOLD:
        periodic_due = cfg.period > 0 and state.step % cfg.period == 0
        return [
            n
            for n in state.nodes.values()
            if _dirty(n) and (periodic_due or len(n.lambda_ctx) >= cfg.threshold)
        ]
