...
def make_batch(
    ...
    # Failing only when *no* issuer qualifies would let a far more insidious
    # case through: if only a handful qualify, the batch is drawn from those
    # few, the contexts in play are concentrated, and every downstream result
    # is biased without any visible symptom. Require most issuers to be
    # usable, and say why when they are not.
    if len(usable) < 0.9 * len(issuer_names):
        counts = sorted(len(v) for v in eligible.values())
        raise ValueError(
            f"only {len(usable)}/{len(issuer_names)} issuers can request "
            f"req_min={cfg.workload.req_min} datasets "
            f"(eligible per issuer: min={counts[0]}, median={counts[len(counts) // 2]}). "
            "With auth_size=1 an issuer may only use its own context's datasets "
            "plus the public ones, so raise n_datasets or public_fraction, or "
            "lower req_min. Proceeding would silently restrict the batch to the "
            "qualifying issuers and concentrate the contexts."
        )
