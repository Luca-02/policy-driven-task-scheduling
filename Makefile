...
init-nodes-count:
	@echo "Initializing cluster with CONTROL_PLANE_COUNT=$(CONTROL_PLANE_COUNT), WORKER_COUNT=$(WORKER_COUNT) and NODE_IMAGE=$(NODE_IMAGE)..."
	@CLUSTER_NAME=$(CLUSTER_NAME) \
		...
		DATASET_SERVICE_LIGHT_MODE=$(DATASET_SERVICE_LIGHT_MODE) \
		CONTEXT_SERVICE_LIGHT_MODE=$(CONTEXT_SERVICE_LIGHT_MODE) \
		bash ./scripts/init-cluster.sh $(NODES_COUNT)
