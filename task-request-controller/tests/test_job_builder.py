...
class TestJobBuilder(unittest.TestCase):
    ...
    def _match_expressions(
        ...
        affinity: client.V1NodeAffinity = job.spec.template.spec.affinity
    ...
    def test_all_zero_or_empty_beta_star_produces_no_affinity(self):
        for beta_star in ({}, {"security": 0}):
            with self.subTest(beta_star=beta_star):
                job = self._build(beta_star=beta_star)
